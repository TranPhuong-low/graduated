...
export default function PersonalityPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 md:p-8 pb-32 relative overflow-hidden">
      <h2 className="text-center text-3xl md:text-4xl font-serif mb-8 md:mb-12 text-slate-800">Hành Trình Kỷ Niệm</h2>
      ...
      <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-8 lg:gap-12 max-w-7xl mx-auto">
        {photos.map((p, index) => (
          <motion.div 
            ...
            key={p.id} 
            className={`relative bg-white p-2 md:p-4 pb-8 md:pb-12 shadow-xl hover:shadow-2xl transition-all duration-500 rounded-sm border-4 md:border-8 border-white border-b-30 md:border-b-60 group ${index % 2 === 0 ? 'rotate-2' : '-rotate-2'} hover:rotate-0`}
          >
            {isAdmin && (
              <button 
            ...
            <img src={p.url} alt={`Kỷ niệm ${p.time}`} className="w-full aspect-square object-cover bg-slate-200" />
